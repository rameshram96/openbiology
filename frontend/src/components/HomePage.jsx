    ...
    <div style={{ minHeight: "100vh", background: "#F7F7F5", fontFamily: "'DM Sans','Segoe UI',sans-serif" }}>
      ...
      <div style={{ background: "#1C1C1C", padding: "0 2.5rem", height: 52, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        ...
        <div style={{ display: "flex", gap: "1rem", alignItems: "center" }}>
          <button onClick={() => navigate("/portfolio")} style={{ background: "transparent", border: "1px solid rgba(255,255,255,0.25)", color: "rgba(255,255,255,0.8)", borderRadius: 7, padding: "0.28rem 0.85rem", fontSize: "0.75rem", cursor: "pointer", fontWeight: 500 }}>
            Developer
          </button>
          <span style={{ color: "rgba(255,255,255,0.3)", fontSize: "0.7rem", fontFamily: "'DM Mono'" }}>v1.0</span>
        </div>
      </div>
      ...
      <div style={{ maxWidth: 1080, margin: "0 auto", padding: "3rem 2rem" }}>
        ...
        <div style={{ ...fadeIn(0), textAlign: "center", marginBottom: "3rem" }}>
          <div style={{ display: "inline-block", background: "#E8F4FD", border: "1px solid #56B4E9", borderRadius: 999, padding: "0.28rem 1rem", marginBottom: "1.1rem" }}>
            <span style={{ color: "#0072B2", fontSize: "0.7rem", fontFamily: "'DM Mono'", letterSpacing: 1.5, textTransform: "uppercase" }}>Statistical Analysis · Plant Science Research</span>
          </div>
        ...
        <div style={{ ...fadeIn(0.1), display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(300px, 1fr))", gap: "1rem", marginBottom: "3rem" }}>
          {modules.map((mod) => {
            ...
            return (
              <div key={mod.id}
                onClick={() => isReady && navigate(`/${mod.id}`)}
                onMouseEnter={() => setHoveredId(mod.id)}
                onMouseLeave={() => setHoveredId(null)}
                style={{ background: "#fff", borderRadius: 14, padding: "1.4rem", border: `1.5px solid ${isHovered ? mod.color : "#E5E5E5"}`, cursor: isReady ? "pointer" : "default", position: "relative", boxShadow: isHovered ? "0 6px 24px rgba(0,0,0,0.09)" : "0 1px 4px rgba(0,0,0,0.05)", transition: "all 0.2s" }}
              >
                ...
                <div style={{ fontSize: "1.5rem", marginBottom: "0.65rem", color: isReady ? mod.color : "#BDBDBD" }}>{mod.icon}</div>
                <h3 style={{ margin: "0 0 0.18rem", fontSize: "0.95rem", fontWeight: 700, color: "#1C1C1C" }}>{mod.title}</h3>
                <p style={{ margin: "0 0 0.6rem", fontSize: "0.68rem", color: isReady ? mod.color : "#AAAAAA", fontFamily: "'DM Mono'" }}>{mod.subtitle}</p>
        ...
        <div style={{ ...fadeIn(0.2), display: "grid", gridTemplateColumns: "1fr 1fr", gap: "1.25rem", marginBottom: "2.5rem" }}>
            ...
            <div style={{ display: "flex", gap: "0.5rem", flexWrap: "wrap", justifyContent: "center", marginBottom: "0.75rem" }}>
              {[
                ...
                <a key={link.label} href={link.url} target="_blank" rel="noreferrer"
                  style={{ padding: "0.28rem 0.7rem", borderRadius: 7, border: `1px solid ${link.color}`, color: link.color, fontSize: "0.7rem", fontWeight: 600, textDecoration: "none" }}>
                  {link.label}
                </a>
              ))}
            </div>
            <button onClick={() => navigate("/portfolio")}
              style={{ padding: "0.4rem 1.1rem", borderRadius: 8, border: "none", background: "#0072B2", color: "#fff", fontSize: "0.75rem", fontWeight: 600, cursor: "pointer" }}>
              View Portfolio →
            </button>
          </div>
          ...
          <div style={{ background: "#fff", borderRadius: 16, padding: "1.75rem", boxShadow: "0 1px 4px rgba(0,0,0,0.06)", border: "1px solid #EEEEEE", display: "flex", flexDirection: "column", alignItems: "center", textAlign: "center" }}>
            ...
            <p style={{ margin: "0 0 1.25rem", fontSize: "0.78rem", color: "#666", lineHeight: 1.75, fontWeight: 300, maxWidth: 300 }}>
            ...
            <div style={{ border: "2px dashed #D0E8FF", borderRadius: 12, padding: "1.1rem", background: "#F8FBFF", width: "100%", maxWidth: 180, textAlign: "center" }}>
              <img src="/upi-qr.png" alt="UPI QR Code"
                onError={e => { e.target.style.display = "none"; e.target.nextSibling.style.display = "block"; }}
                style={{ width: "100%", borderRadius: 8 }} />
              <div style={{ display: "none", padding: "0.75rem 0" }}>
                <div style={{ fontSize: 32, marginBottom: "0.35rem" }}>📱</div>
                <p style={{ margin: 0, fontSize: "0.67rem", color: "#AAA" }}>Add <code>upi-qr.png</code><br />to <code>frontend/public/</code></p>
              </div>
              <p style={{ margin: "0.55rem 0 0", fontSize: "0.7rem", color: "#0072B2", fontWeight: 600 }}>Scan to Support via UPI</p>
            </div>
          </div>
        </div>
